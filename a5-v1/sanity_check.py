...
####This just check the dimensionality and output
...
def question_1h_sanity_check():
    ...
    word_emb = 256
    inpt = torch.zeros(word_emb)
    ####call the highway layer
    highway_layer = Highway(word_emb)
    ###initialize the weight
    reinitialize_layers(highway_layer)


    inpt = torch.zeros(word_emb)
    output = highway_layer.forward(inpt)
    ###print(highway_layer.proj(inpt).size()[0])
    ###print(highway_layer.gate(inpt).size()[0])
    assert(highway_layer.proj(inpt).size()[0] == word_emb)
    assert(highway_layer.gate(inpt).size()[0] == word_emb)
    assert(output.size()[0] == word_emb)
    ####calculate the result manually
    W_test = torch.Tensor(word_emb,word_emb)
    W_test = W_test.fill_(0.3)
    B_test = torch.Tensor(word_emb,word_emb)
    B_test = B_test.fill_(0.1)
    X_proj_test = F.relu(W_test * inpt + B_test)
    X_gate_test = F.sigmoid(W_test * inpt + B_test)
    output_test = X_gate_test * X_proj_test + (1-X_gate_test) * inpt
    ###print(output_test)
    ###print(output)
...
#####The above part needs to be commented out later    

####This part is for 1g sanity check
####This part only check the dimension
...
#### The above is self-written test code
    echar = 50
    mword = 21
    eword = 256
    inpt = torch.rand(1,echar,mword)
    #print(inpt)
    ####call the convolutional layer
    conv_net = CNN(echar,mword,eword)
    ###initialize the weight
    reinitialize_layers(conv_net)
    ####print out the shape of
    #### x_conv and the output
    output = conv_net.forward(inpt)
    #print(conv_net.conv(inpt).size())
    #print(output.size())
    ###also check if it is doing the max pooling
    #print(conv_net.conv(inpt))
    #print(output)
...
def main():
    ...
    ####This is the sanity check for 1h
